...
#
# ---
# Hmmm... one of the tests still doesn't pass
...
def maxDmg(HD):
   ...
   maxd, i, j = maxs[0]
   for k in HD[2:]:
      maxs.extend([(maxk, ki, kj) for maxd, i, j in maxs
                                  for maxk, ki, kj in [mm(k, i), mm(k, j)]])
      maxs.sort(key=lambda x: (-x[0], x[1], x[2]))
      maxd, i, j = maxs[0]
      for k in range(len(maxs)-1, 0, -1):
         maxd, i, j = maxs[k-1]
         maxk, p, q = maxs[k]
         if maxk < maxd or (i, j) == (p, q):
            maxs.pop(k)
   return maxd
