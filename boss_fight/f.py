...
def maxDmg(HD, rank):
   ...
   if len(rank) == 1 and i != j:
      ...
      i = i[1] // i[0], i[0]
      return max([hh for j in HD if i != j for hh, _, _ in [m(i, j), m(j, i)]])
   ...
   for ch, cd in HD:
      ...
      if (cd, chd) == n:
         ...
         ns = [rank[lo][-1], rank[lo+1][0:2] if lo + 1 < len(rank) else None]
